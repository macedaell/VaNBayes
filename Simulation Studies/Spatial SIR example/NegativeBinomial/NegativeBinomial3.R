...
init_NegativeBinomial3 <- function(p,hidden_layers,init_mn=0,init_sd=1,sigma=0.01){
...
predict_NegativeBinomial3 <- function(w,x){
...
loss_NegativeBinomial3 <- function(w,x,y){
  l <- predict_NegativeBinomial3(w,x)
  l <- -sum(dnbinom(y,mu=l$mu,size=l$mu^2/(l$sigma^2 - l$mu),log=TRUE))
  return(l)}


grad_NegativeBinomial3 <- function(w,x,y,transfer_learning = FALSE,priorweights = rep(1,length(y))){
  ...
  chain_rule_m3 = (2*m3*exp(s3)^2 - m3^2)/(exp(s3)^2 - m3)^2
  chain_rule_s3 = -m3^2*2*exp(s3)^2/(exp(s3)^2 - m3)^2
  
  dldm3  <- digamma(y + m3^2/(exp(s3)^2-m3))*chain_rule_m3 - digamma(m3^2/(exp(s3)^2-m3))*chain_rule_m3 - y/(exp(s3)^2-m3) + m3/(exp(s3)^2-m3) + log(m3)*chain_rule_m3 - 2*chain_rule_m3*s3
  dldm3  <- -dldm3
  dlds3  <- digamma(y + m3^2/(exp(s3)^2 - m3))*chain_rule_s3 - digamma(m3^2/(exp(s3)^2-m3))*chain_rule_s3 + 2*y*(exp(s3)^2/(exp(s3)^2-m3))-2*y + log(m3)*chain_rule_s3 - 2*m3^2/(exp(s3)^2-m3) -2*s3*chain_rule_s3
  dlds3  <- -dlds3
...
if(FALSE){ # Check gradient
 ...
 w   <- init_NegativeBinomial3(p,c(L1,L2),init_mn = mean(y), init_sd = max(var(y), mean(y)))
 test <- predict_NegativeBinomial3(w,x)
 l0  <- loss_NegativeBinomial3(w,x,y)
 ...
 numgrad <- function(w,x,y,k,l0,eps){
   ...
   if(is.vector(g)){
      for(i in 1:length(g)){
        We         <- w
        We[[k]][i] <- We[[k]][i] + eps
        g[i]       <- loss_NegativeBinomial3(We,x,y)
      }
   }
   if(is.matrix(g)){
      for(i in 1:nrow(g)){for(j in 1:ncol(g)){
        ...
        g[i,j] <- loss_NegativeBinomial3(We,x,y)
      }}
   }
 return((g-l0)/eps)}
 ...
 g2 <- grad_NegativeBinomial3(w,x,y)
