...
# 2 to 9
selection = 1:12 # 2:12 # 2:6 is the current
N_vec = N_vec[selection] # Brian liked something like: c(100, 500, 1000, 2000, 3000, 4000, 5000, 6000)

alt_simulation_results = simulation_results
alt_simulation_results = simulation_results[,,,selection]
...
for (k in 1:length(N_vec)){
  # coverage
  parameter_coverage[,k] = c(sum((alt_simulation_results[,1,1,k] < true_parameters[1]) & (true_parameters[1] < alt_simulation_results[,1,3,k]))/nsims,
                             sum((alt_simulation_results[,2,1,k] < true_parameters[2]) & (true_parameters[2] < alt_simulation_results[,2,3,k]))/nsims,
                             sum((alt_simulation_results[,3,1,k] < true_parameters[3]) & (true_parameters[3] < alt_simulation_results[,3,3,k]))/nsims)
  # MAD
  parameter_MAD[,k] = c(mean(abs(alt_simulation_results[,1,2,k] - true_parameters[1])), 
                        mean(abs(alt_simulation_results[,2,2,k] - true_parameters[2])), 
                        mean(abs(alt_simulation_results[,3,2,k] - true_parameters[3])))
  
  # SD
  parameter_SDAD[,k] = c(sd(abs(alt_simulation_results[,1,2,k] - true_parameters[1])), 
                         sd(abs(alt_simulation_results[,2,2,k] - true_parameters[2])), 
                         sd(abs(alt_simulation_results[,3,2,k] - true_parameters[3])))
...
VaNBayes_medians = alt_simulation_results[,,2,]
...
bayesflow_coverage = read.csv("./bayesflow_parameter_coverage.csv", header = FALSE)
bayesflow_coverage = as.matrix(bayesflow_coverage[,selection])
bayesflow_MAD = read.csv("./bayesflow_parameter_MAD.csv", header = FALSE)
bayesflow_MAD = as.matrix(bayesflow_MAD[,selection])
bayesflow_SDAD = read.csv("./bayesflow_parameter_SDAD.csv", header = FALSE)
bayesflow_SDAD = as.matrix(bayesflow_SDAD[,selection])
...
for (j in 1:length(true_parameters)){ # true lambda, I0, psi
  plot(x = 1:length(N_vec),
       ...
       #                bayesflow_MAD[j,] + 2*bayesflow_SDAD[j,])))
  ylim = range(c(parameter_MAD[j,], bayesflow_MAD[j,],0)))
lines(x = 1:length(N_vec),
  ...
  if (j == 1){
    legend("right", inset=c(0,-.1),xpd=TRUE, # need to adjust the first inset number
...
for (j in 1:length(true_parameters)){ # true lambda, I0, psi
  plot(x = 1:length(N_vec),
       ...
       ylab = "coverage",
       xlim = range(c(1:length(N_vec)-0.25, 1:length(N_vec)+0.25)),
       # ylim = range(c(parameter_coverage[j,] - 2*parameter_SDAD[j,], 
       #                parameter_coverage[j,] + 2*parameter_SDAD[j,], 
       #                bayesflow_coverage[j,] - 2*bayesflow_SDAD[j,],
       #                bayesflow_coverage[j,] + 2*bayesflow_SDAD[j,])))
       ylim = range(c(parameter_coverage[j,], bayesflow_coverage[j,],0)))
  lines(x = 1:length(N_vec),
        y = bayesflow_coverage[j,],
        lwd = 2, 
        lty = 1,
        col = "darkblue",)
  abline(h = 0, col = "red")
