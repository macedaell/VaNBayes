#####################################################################
# This code defines the loss function and its gradient for the heterogeneous normal
#      density with a two-layer network for the mean and log standard deviation.
#####################################################################
...
# Loss function
loss_hetnorm <- function(w,x,y,weights=1){
   l <- predict_hetnorm(w,x)
   l <- -sum(weights*dnorm(y,l$mu,l$sigma,log=TRUE))
return(l)}

# Gradient of the loss function
grad_hetnorm <- function(w,x,y,weights=1){
...
if(FALSE){ # Check gradient
 ...
 y       <- rnorm(n)
 weights <- rgamma(n,2,4)
 w       <- init_hetnorm(p,L1,L2)
 l0      <- loss_hetnorm(w,x,y,weights)

 numgrad <- function(w,x,y,k,l0,eps,weights){
   g <- w[[k]]
   if(is.vector(g)){
      for(i in 1:length(g)){
        We         <- w
        We[[k]][i] <- We[[k]][i] + eps
        g[i]       <- loss_hetnorm(We,x,y,weights)
      }
   }
   if(is.matrix(g)){
      for(i in 1:nrow(g)){for(j in 1:ncol(g)){
        ...
        g[i,j] <- loss_hetnorm(We,x,y,weights)
      }}
   }
 return((g-l0)/eps)}
 ...
 eps <- 10^(-8)
 g1   <- w
 for(k in 1:12){g1[[k]]<-numgrad(w,x,y,k,l0,eps,weights)}
 g2 <- grad_hetnorm(w,x,y,weights)
 print(g1[[10]])
 print(g2[[10]])
}
