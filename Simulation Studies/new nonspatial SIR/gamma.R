...
# this is fine
init_gamma <- function(p,L1,L2,init_a=1,init_b=1,sigma=0.01){
 w       <- list()
 w[[1]]  <- sigma*rnorm(L1)
 w[[2]]  <- sigma*matrix(rnorm(p*L1),p,L1)
 w[[3]]  <- sigma*rnorm(L2)
 w[[4]]  <- sigma*matrix(rnorm(L1*L2),L1,L2)
 w[[5]]  <- log(init_a) + sigma*rnorm(1)
 w[[6]]  <- sigma*rnorm(L2)
 w[[7]]  <- sigma*rnorm(L1)
 w[[8]]  <- sigma*matrix(rnorm(p*L1),p,L1)
 w[[9]]  <- sigma*rnorm(L2)
 w[[10]] <- sigma*matrix(rnorm(L1*L2),L1,L2)
 w[[11]] <- log(init_b) + sigma*rnorm(1)
 w[[12]] <- sigma*rnorm(L2)
return(w)}


# output... also fine
predict_gamma <- function(w,x){
...
grad_gamma <- function(w,x,y){
   ...
   # # rate parameter
   # dldm3  <- -exp(m3)*s3 + digamma(exp(m3))*exp(m3)-exp(m3)*log(y)
   # dldm3  <- -dldm3
   # dlds3  <- exp(m3) - exp(s3)*y
   # dlds3  <- -dlds3
