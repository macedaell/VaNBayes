...
for (k in 1:length(N_vec)){
  ...
  bayesflow_medians[,,k] = as.matrix(read.csv(paste0("./results_PCA2/bayesflow_posterior_medians_N_",N,".csv"), header = FALSE))
}
bayesflow_parameter_coverage = read.csv("./results_PCA2/bayesflow_parameter_coverage.csv", header = FALSE)
bayesflow_parameter_coverage = as.matrix(bayesflow_parameter_coverage[,1:length(N_vec)])
bayesflow_MAD = read.csv("./results_PCA2/bayesflow_parameter_MAD.csv", header = FALSE)
bayesflow_MAD = as.matrix(bayesflow_MAD[,1:length(N_vec)])
bayesflow_SDAD = read.csv("./results_PCA2/bayesflow_parameter_SDAD.csv", header = FALSE)
