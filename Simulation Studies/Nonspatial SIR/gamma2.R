...
# NOTE: the scale parametrization is WAAAAY more numerically stable than rate
# almost like we can't even get a good rate parametrization
act  <- function(x){pmax(x,0)}
actp <- function(x){x>0}

# this is fine
init_gamma2 <- function(p,hidden_layers,init_a=1,init_b=1,sigma=0.01){
  ...
  W[[bias_index - 2]][1,] = W[[bias_index - 2]][1,] + log(init_a)
  W[[bias_index - 2]][2,] = W[[bias_index - 2]][2,] + log(init_b)
return(W)}


# output... also fine
predict_gamma2 <- function(w,x){
  ...
  # print(dim(x))
return(list(a=exp(x[,1]),b=exp(x[,2])))}

# this is good
loss_gamma2 <- function(w,x,y){
   l <- predict_gamma2(w,x)
   # l <- -sum(dgamma(y,shape = l$a, rate = l$b,log=TRUE))
   l <- -sum(dgamma(y,shape = l$a, scale = l$b,log=TRUE))
   return(l)}




grad_gamma2 <- function(w,x,y,transfer_learning=FALSE,priorweights = rep(1,length(y))){
  ...
  # scale parameter
  dldm3 <- -digamma(exp(m3))*exp(m3) - s3*exp(m3) + exp(m3)*log(y)
  dldm3 <- -dldm3
  dlds3 <- -exp(m3) + y*exp(-s3)
  dlds3 <- -dlds3
...
if(FALSE){ # Check gradient... looks good
 ...
 w   <- init_gamma2(p,c(L1,L2))
 l0  <- loss_gamma2(w,x,y)
