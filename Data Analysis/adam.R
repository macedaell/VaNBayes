adam <- function(w, x, y, loss, grad, priorweights,
                 lr = 0.0001, batchsize = 100, epochs = 500, propval = 0.2,
                 beta1=0.9, beta2=0.999, epsilon=0.1^8,
                 verbose=2,early_stop=5){
            
  # https://arxiv.org/pdf/1412.6980.pdf

  val   <- seq(0,1,length=length(y)) > 1-propval # split the dataset according to propval
  
  # validation set
  x_val <- x[val,]
  y_val <- y[val]
  priorweights_val <- priorweights[val]
  
  # training set
  x     <- x[!val,]
  y     <- y[!val]
  priorweights <- priorweights[!val]
  
  n     <- length(y) # number of responses
  nb    <- floor(n/batchsize) # number of batches
  bs    <- rep(1:nb,length=n)[1:n] # where to allocate  
  ...
  for(iter in 1:epochs){
    ...
    if(!stop){
     ...
     # handles NA cases
     if(is.na(loss(w,x,y,priorweights))){w<-w0;m<-m0;v<-v0}
     if(is.na(loss(w,x_val,y_val,priorweights_val))){w<-w0;m<-m0;v<-v0}
     
     
     
     train_loss[iter] <- loss(w,x,y,priorweights)/length(y)
     val_loss[iter]   <- loss(w,x_val,y_val,priorweights_val)/length(y_val)
     if(val_loss[iter]==min(val_loss[1:iter])){bestw<-w}   

     if(verbose == 1){
       # print(c(iter,train_loss[iter],val_loss[iter], iter/epochs))
     }
     if(verbose == 2){
       # print(val_loss)
       # print("")
  ...
  if (!because_early){
    # print("FINISHED EPOCHS, DIDN'T CONVERGE")
  }
  
  # cat("The minimum loss value is", min(val_loss[1:iter]))
  # print("")
...
stdq <- function(x,q,low=-1,high=1){
   m         <- length(q)
   x[x<q[1]] <- q[1] # ensure the maximum
   x[x>q[m]] <- q[m] # ensure the minimum
   U         <- 1+0*x # initialize U
   for(j in 2:(m-1)){U[x>q[j]] <- j} # get quantile each element of X falls into
   # print(sort(U))
   U         <- U+(x-q[U])/(q[U+1]-q[U]) # transform each 
   # print(U)
   U[is.nan(U)] = q[1]
