...
init_hetnorm <- function(p,L1,L2,init_mn=0,init_sd=1,sigma=0.01){
 ...
 # predict marginal standard deviation
 w[[7]]  <- sigma*rnorm(L1)
...
loss_hetnorm <- function(w,x,y,priorweights){
   l <- predict_hetnorm(w,x)
   l$sigma[l$sigma == -Inf] = 0
   l$sigma[l$sigma == Inf] = max(l$sigma[l$sigma != Inf])
   # l <- -sum(priorweights*dnorm(y,l$mu,l$sigma,log=TRUE))
